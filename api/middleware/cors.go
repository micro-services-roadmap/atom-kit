...
func CorsByRules() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if whitelist == nil &&
			kg.C.Cors.Mode == "strict-whitelist" &&
...
func checkCors(currentOrigin string) *aconfig.CORSWhitelist {
	for _, whitelist := range kg.C.Cors.Whitelist {
		// 遍历配置中的跨域头，寻找匹配项
